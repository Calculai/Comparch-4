...
int main(int argc, char **argv)
{
    ...
    {
        void *block = malloc(request_size);
        if (block == NULL)
        {
            break;
        }
        ...
        print_step_info("malloc", count, request_size, total_retained, start);
    ...
    printf("\n[malloc] finished after %zu successful allocations\n", count);
    printf("[malloc] largest single allocation: %s\n", last_buffer);
    printf("[malloc] total retained memory:   %s\n", total_buffer);
    printf("[malloc] elapsed CPU time:        %.2fs\n", elapsed_seconds(start));
